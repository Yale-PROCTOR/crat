...
use rustc_ast::{
    AttrKind, Crate, Expr, ExprKind, Extern, FnRetTy, Item, ItemKind, LocalKind, PatKind, Stmt,
    StmtKind, Ty, TyKind, mut_visit, mut_visit::MutVisitor, ptr::P, visit, visit::Visitor as _,
};
...
fn apply_target_signature(
    ...
    {
        if raw_decision_matches_ast_type(kind, output) {
            return;
        }
