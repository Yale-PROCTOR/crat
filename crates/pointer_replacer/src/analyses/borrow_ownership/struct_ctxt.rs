...
impl<'tcx> StructCtxt<'tcx> {
    ...
    pub fn new(tcx: TyCtxt<'tcx>, structs: &[DefId]) -> Self {
        ...
        TarjanScc::new().run(&graph, |nodes| {
            post_order.extend(nodes.iter().map(|&idx| graph[idx]));
        });
            ...
            for field_def in adt_def.all_fields() {
                ...
                if ptr_depth >= max_ptr_depth {
                    ...
                    for _ in 0..max_ptr_depth {
                        ...
                        leaf_ext_ty = leaf_ext_ty.builtin_deref(true).unwrap();
                    }
...
mod tests {
    ...
    use crate::analyses::borrow_ownership::ptr::Measurable;
    ...
    #[test]
    fn test1() {
        run_compiler(TEXT1, |tcx| {
            let structs = collect_structs(tcx);
            let struct_ctxt = StructCtxt::new(tcx, &structs);
            ...
            macro_rules! define_structs {
                ($( $x: ident ),* $(,)?) => {
                    $(
                        let &$x = structs
                            .iter()
                            .find(|&&did| has_name(tcx, did, stringify!($x)))
                            .unwrap();
                    )*
                };
            }
    ...
    #[test]
    fn test3() {
        run_compiler(TEXT3, |tcx| {
            let structs = collect_structs(tcx);
            let mut struct_ctxt = StructCtxt::new(tcx, &structs);
            let &node = struct_ctxt
                .post_order
                .iter()
                .find(|&&did| has_name(tcx, did, "S"))
                .unwrap();
            let node = tcx.adt_def(node);
