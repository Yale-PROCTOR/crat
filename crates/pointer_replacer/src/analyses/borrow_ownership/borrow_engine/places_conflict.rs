// @generated MIRROR — DO NOT EDIT. Byte-identical (below the boundary) to production
// `crates/pointer_replacer/src/analyses/borrow/places_conflict.rs` @ fc3bd4cf.
// Mirrored LEAF (never diverges). Its drift can HIDE from the equivalence differential — a
// subtly-changed place-conflict could agree on every current fixture yet differ elsewhere — so it
// is guarded by the `fork_sync_places_conflict` byte-compare tripwire in `mod.rs`. To update:
// re-copy from production verbatim (keep this header).
