...
use crate::analyses::borrow_ownership::ssa::{
...
impl<'rn, 'tcx: 'rn> Renamer<'rn, 'tcx> {
    ...
    where Infer: InferMode<'rn, 'db, 'tcx> + 'rn {
        ...
        self.body.basic_blocks.indices().for_each(|bb| {
            if let Some(dom) = dominators.immediate_dominator(bb) {
        ...
        Infer: InferMode<'rn, 'db, 'tcx>,
    {
        ...
        match rhs {
            ...
            | Rvalue::WrapUnsafeBinder(
                operand @ Operand::Copy(rhs) | operand @ Operand::Move(rhs),
                _,
            ) => {
                let lhs_consume =
                    consume_place_at::<Infer>(lhs, self.body, location, self, infer_cx);
            ...
            | Rvalue::RawPtr(rustc_middle::mir::RawPtrKind::Mut, rhs) => {
                let lhs_consume =
                    consume_place_at::<Infer>(lhs, self.body, location, self, infer_cx);
                let rhs_consume =
                    consume_place_at::<Infer>(rhs, self.body, location, self, infer_cx);
