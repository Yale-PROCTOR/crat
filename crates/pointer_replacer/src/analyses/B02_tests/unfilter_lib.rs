    ...
    pub mod lib {
        ...
        unsafe extern "C" fn cp_ptr(s: *mut cp_state_t) -> *mut core::ffi::c_char {
            if (*s).bits_left & 7 as core::ffi::c_int == 0 {
                ...
                __assert_fail(b"!(s->bits_left & 7)\0" as *const u8 as
                    ...
                    b"/home/ubuntu/Test-Corpus/Public-Tests/B02_organic/unfilter_lib/src/unfilter_lib/test_case/src/lib.c\0"
        ...
        unsafe extern "C" fn cp_peak_bits(
            ...
            if (*s).count < num_bits_to_read {
                if (*s).word_index < (*s).word_count {
                    ...
                    (*s).word_index += 1;
                    let word: uint32_t = *((*s).words).offset(fresh13 as isize);
        ...
        unsafe extern "C" fn cp_build(
            ...
            while i < sym_count {
                ...
                if len != 0 as core::ffi::c_int {
                    ...
                    let code: uint32_t = fresh14 as uint32_t;
                    let fresh15 = first[len as usize];
                    first[len as usize] += 1;
                    let slot: uint32_t = fresh15 as uint32_t;
                    ...
                    if ((*s).out).offset(1 as core::ffi::c_int as isize) > (*s).out_end {
                        cp_error_reason =
                            b"Attempted to overwrite out buffer while outputting a symbol.\0"
                                as *const u8
                                as *const core::ffi::c_char;
                        current_block = 8920533936274006012;
                        break;
                    } else {
        ...
        pub unsafe extern "C" fn cp_inflate(
            ...
            loop {
                ...
                match btype {
                    0 => {
                        if cp_stored(s) == 0 {
                            current_block = 9231435637045189962;
                            break;
                        }
                    }
                    1 => {
        ...
        pub unsafe extern "C" fn unfilter(
            ...
            while y < h {
                let fresh4 = *raw;
                raw = raw.offset(1);
                match fresh4 as core::ffi::c_int {
                    0 => {}
                    1 => {
...
fn ownership_analysis_runs() {
    run_ownership_case("unfilter_lib", SOURCE);
}
