...
///
/// ⚠ **Was a TWO-LAYER instrument until M-3** (2026-08-18). Its span side is
/// gone with the span layer; what remains is the AST layer's claimed edits, its
/// per-edit kind/owner/span/text, and the emitted-text-vs-substrate comparison.
/// The two-layer comparison it used to provide is not replaceable — there is
/// one layer — and that is a real loss of diagnostic reach, recorded rather
/// than papered over: it is how I-31 was found.
///
...
pub(crate) fn edit_dump(
    ...
    let keys: std::collections::BTreeSet<&super::plan::FileKey> = files.keys().collect();
    ...
    for key in keys {
        ...
        let _ = writeln!(o, "-- {key:?}");
        let _ = writeln!(o, "   emitted = {}", verdict(files.get(key)));
        if let Some(dir) = texts_out {
            ...
            for (label, text) in [("emitted", files.get(key))] {
    ...
    fn the_seam_family_survives_the_projection_in_both_directions() {
        rustc_span::create_default_session_globals_then(|| {
            let edit = |family| super::super::decision::seam::SeamEdit {
                ...
                bridge: super::super::bridge_receipt::BridgeSitePlan::local(
                    rustc_hir::def_id::CRATE_DEF_ID,
                    rustc_hir::def_id::CRATE_DEF_ID,
                    "c",
                    "arg0",
