...
/// declaration can be inserted by [`explicit_declaration`] and no site of the
/// local is a pending sibling-overlap row.
pub(crate) fn admits(ctx: &Ctx<'_, '_>, subject: &Subject, decision: &Decision) -> bool {
    admissible(ctx, subject, decision) && !pending_sibling(ctx, subject)
}

/// **Relay 018 §1 (R304-2).** The declaration would be inserted, but the
/// local's boundary site is a pending sibling-overlap row: a stated hold, so
/// the local stays raw under its own typed reason.
pub(crate) fn pending_sibling_hold(
    ctx: &Ctx<'_, '_>,
    subject: &Subject,
    decision: &Decision,
) -> bool {
