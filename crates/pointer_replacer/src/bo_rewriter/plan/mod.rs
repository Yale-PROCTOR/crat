...
/// whole-call `pair` site owned by the callee class, plus one `pair:arg{N}`
/// site per selected view — over another class's edit inside the call.
///
/// `A5RawGraftVisitor` walks the call's children first and pretty-prints the
/// transformed call, then substitutes only the selected views' arguments with
/// raw views rendered from the source form (`pair_raw_view_expression`:
/// `Slice → .as_ptr()`, `Opt → .as_deref()…`, `Ref → from_ref`, a raw source
/// passes through into `let t: <target> = <arg>`, whose `*mut T → *const T`
/// coercion is the mutability cast). Two cases therefore compose:
///
/// * the edit sits in an UNSELECTED argument — it survives verbatim in the
///   rendered wrapper;
/// * the edit is the caller's own `c` bridge at EXACTLY a selected view's
///   interval, converting the same source form to the same raw target, and
///   the view is renderable (`EditReady`) — the wrapper overwrites it with an
///   equivalent rendering (the 100 `a5-site-proof-t2-fallback` × `raw-cast-const`
///   / `option-to-raw-null-map` / … rows at the batch-6 frame).
///
/// Everything else still collides: an edit partly inside a selected view, a
/// different source form (a different conversion), an unrenderable view (the
/// inner bridge is then the only rendering), or a wrapper without recorded
/// view sites (selected and unselected cannot be told apart). The outer
/// depends on the inner (the existing containment discipline).
fn a5_wrapper_composition(
    left: &ClassSite,
    right: &ClassSite,
    all_sites: &[ClassSite],
) -> Option<(SignatureClassId, SignatureClassId)> {
    const KIND: &str = "a5-site-proof-t2-fallback";
    let composable = |outer: &ClassSite, inner: &ClassSite| {
        if outer.key.arm != "pair"
            || outer.key.bridge_kind != KIND
            || outer.key.owner_class == inner.key.owner_class
        ...
        let same_call = |site: &ClassSite| {
            site.key.owner_class == outer.key.owner_class
                && site.key.caller == outer.key.caller
                && site.key.arm == "pair"
                && site.key.bridge_kind == KIND
                && site.key.file == outer.key.file
        };
        // The whole-call site of this wrapper: `outer` itself, or the `args=`
        // sibling containing a view `outer`.
        let whole = if outer.key.position.starts_with("args=") {
            Some(outer)
        } else {
            all_sites.iter().find(|site| {
                same_call(site)
                    && site.key.position.starts_with("args=")
                    && site.key.lo <= outer.key.lo
                    && outer.key.hi <= site.key.hi
            })
        };
        let Some(whole) = whole else {
            return false;
        };
        if !(whole.key.lo < inner.key.lo && inner.key.hi < whole.key.hi) {
            return false;
        }
        let views = all_sites
            .iter()
            .filter(|site| {
                same_call(site)
                    ...
                    && !site.key.position.starts_with("args=")
                    && whole.key.lo <= site.key.lo
                    && site.key.hi <= whole.key.hi
            })
            .collect::<Vec<_>>();
        if views.is_empty() {
            return false;
        ...
        match views
            .iter()
            .find(|view| !(inner.key.hi <= view.key.lo || view.key.hi <= inner.key.lo))
        {
            None => true,
            Some(view) => {
...
mod wave3_class_tests {
    ...
    /// **Relay 042 / report 028 §4 — the class-2440 shape, INTRA-class.**
    ///
    /// brotli's `BrotliCompressBufferQuality10` plans the C arm's borrow view
    /// of a Box owner at `11456136..11456181` and that owner's own
    /// `box-expression` at `11456141..11456181` — the same class, the outer
    /// exceeding the inner by exactly the five bytes of `&mut `. Two sites over
    /// one interval in one class is `intra-class-interval-overlap` unless the
    /// containment is a named composition; it is one, because the pair is a
    /// parent `AddrOf` node and its operand, and the seam pass builds its
    /// adapter AROUND the node it finds, after the use pass grafted the
    /// operand.
    ///
