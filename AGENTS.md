...
## Scope
- Active implementation focus is limited to `crates/pointer_replacer/src/rewriter`.
- Read-only use of ownership and output-parameter analysis modules is allowed when making rewriter decisions.
...
- Execution order is: `M0 -> M1 -> M2 -> M3A -> M3B -> M4 -> M5`.
...
- Required order (do not reorder):
  ...
  2. Always edit `crates/pointer_replacer/src/rewriter/CANONICAL_REWRITER_SPEC.md` in the same change set.
...
- [x] M1 - Post-spec plumbing foundation
- [ ] M2 - Decision logic extension (`OptBox`, `OptBoxedSlice`)
- [ ] M3A - Core rewrite implementation for box kinds
- [ ] M3B - Conditional struct support (`Default`, `take`)
- [ ] M4 - B02 rewrite compile gate
- [ ] M5 - Unsafe minimization and hardening
...
- Date: 2026-03-05
  ...
  - crates/pointer_replacer/src/tests.rs
  Behavior changes:
  - Rewriter analysis plumbing now carries `output_params`, `ownership_schemes`, and `ownership_solidified` in `Analysis<'tcx>` without changing `PtrKind` decision branches.
  - Ownership artifacts are computed through a non-fatal path (`compute_ownership_artifacts`) and do not affect rewrite output in M1.
  - Added B02 rewrite artifact instrumentation gated by `B02_REWRITE_ARTIFACT_PATH` and `B02_REWRITE_ARTIFACT_ONLY`, including panic-safe artifact capture for before/after byte comparison.
  - Added invariant test `ownership_analysis::rewriter_analysis_artifacts_cover_all_rewritten_functions` to ensure `output_params`, `ownership_schemes`, and `ownership_solidified` are populated for each rewritten function under test helper construction.
  - Canonical rewriter spec now records M1 non-goals and the current ownership/output-parameter plumbing behavior.
  Tests run:
  - command: B02_REWRITE_ARTIFACT_ONLY=1 B02_REWRITE_ARTIFACT_PATH=/tmp/m1_b02_pre_a.txt cargo test -p pointer_replacer analyses::B02_tests -- --test-threads=1
    result: pass
  - command: B02_REWRITE_ARTIFACT_ONLY=1 B02_REWRITE_ARTIFACT_PATH=/tmp/m1_b02_pre_b.txt cargo test -p pointer_replacer analyses::B02_tests -- --test-threads=1
    result: pass
  - command: cmp -s /tmp/m1_b02_pre_a.txt /tmp/m1_b02_pre_b.txt
    result: pass
  - command: B02_REWRITE_ARTIFACT_ONLY=1 B02_REWRITE_ARTIFACT_PATH=/tmp/m1_b02_post.txt cargo test -p pointer_replacer analyses::B02_tests -- --test-threads=1
    result: pass
  - command: cmp -s /tmp/m1_b02_pre_a.txt /tmp/m1_b02_post.txt
    result: pass
  - command: cargo test -p pointer_replacer ownership_analysis::malloc_source_marks_return_as_owning
    result: pass
  - command: cargo test -p pointer_replacer ownership_analysis::free_sink_clears_ownership_before_return
  ...
  - command: cargo test -p pointer_replacer analyses::B02_tests
    result: pass
  - command: cargo test -p pointer_replacer ownership_analysis::rewriter_analysis_artifacts_cover_all_rewritten_functions
    result: pass
  - command: cargo test -p pointer_replacer
    result: pass
  Worker runs:
  - role: coverage
    run_id: 019cbe35-344b-7c03-99f3-7b5b07bb20fb
    verdict: pass
  - role: correctness
    run_id: 019cbe35-3453-7f90-bf4b-4b407d981f71
    verdict: pass
  - role: implementation-readiness
    run_id: 019cbe35-3499-7190-935f-dcb9a3ab7684
    verdict: pass
  Blockers:
  - role: none
    summary: none
  ...
  Notes:
  - Gate result: PASS.
  - M1 non-goal was preserved: ownership/output-parameter facts are plumbed but not consumed by pointer decision logic.
```
...
### M1: Post-Spec Plumbing Foundation (retained)
Goal:
...
### M3A: Core Rewrite Implementation
Goal:
...
### M3B: Conditional Struct Support
Goal:
- Add struct support required by M3A compile failures:
...
4. `cargo test -p pointer_replacer analyses::B02_tests`
5. `cargo test -p pointer_replacer`
